...
export function useGraphics(
  ...
  function updateBuffers() {
    if (!gl || !glData || !textureAtlas) return
  ...
  function draw() {
    if (!canvas || !gl || !glData || !textureAtlas) return
...

    {
      ...
      const offset = 0 // how many bytes inside the buffer to start from
      gl.bindBuffer(gl.ARRAY_BUFFER, glData.textureCoordBuffer)
      gl.vertexAttribPointer(glData.programData.attributes.aTextureCoord, num, type, normalize, stride, offset)
